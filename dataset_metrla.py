...
class MetrLA_Dataset(Dataset):
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            "observed_data": torch.tensor(ob_data).float(),
            "observed_mask": torch.tensor(ob_mask).float(),
            "gt_mask": gt_mask,
...
if __name__ == "__main__":
    train_loader, valid_loader, test_loader, scaler, mean_scaler = get_dataloader(
        batch_size=64, device="cuda:0", missing_pattern="block",
        is_interpolate=True, num_workers=4,
        target_strategy="random",
    )
    
    s = next(iter(train_loader))
    print(s["observed_data"].shape)
    print()
